...
from sintetizador.model.execution.inviabilidade import Inviabilidade, InviabilidadeTI, InviabilidadeEV, InviabilidadeHV, InviabilidadeHE, InviabilidadeHQ, InviabilidadeRE, InviabilidadeDEFMIN, InviabilidadeFP, InviabilidadeDeficit
...
class ExecutionSynthetizer:
    ...
    def __init__(self, uow: AbstractUnitOfWork) -> None:
        ...
        self.__rules: Dict[Variable, pd.DataFrame] = {
    ...
    def _resolve_convergence(self) -> pd.DataFrame:
        ...
        df_processed.drop(columns=["Tot. Def. Niv. Seg. (MWmes)"], inplace=True)
        return df_processed
    ...
    def _resolve_inviabilidades_codigo(self) -> pd.DataFrame:
        inviabs_codigo = [i for i in self.inviabilidades if type(i) in self.__class__.INVIABS_CODIGO]
        iteracoes: List[int] = []
        cenarios: List[int] = []
        estagios: List[int] = []
        codigos: List[int] = []
        ...
        return pd.DataFrame(data={"Iteracao": iteracoes, "Cenarios": cenarios, "Estagio": estagios, "Codigo": codigos, "Violacao": violacoes, "Unidade": unidades})
    ...
    def _resolve_inviabilidades_patamar(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(data={"Iteracao": iteracoes, "Cenarios": cenarios, "Estagio": estagios, "Codigo": codigos, "Violacao": violacoes, "Unidade": unidades, "Patamar": patamares})
    ...
    def _resolve_inviabilidades_patamar_limite(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(data={"Iteracao": iteracoes, "Cenarios": cenarios, "Estagio": estagios, "Codigo": codigos, "Violacao": violacoes, "Unidade": unidades, "Patamar": patamares, "Limite": limites})
    ...
    def _resolve_inviabilidades_limite(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(data={"Iteracao": iteracoes, "Cenarios": cenarios, "Estagio": estagios, "Codigo": codigos, "Violacao": violacoes, "Unidade": unidades, "Limite": limites})
    ...
    def _resolve_inviabilidades_submercado_patamar(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(data={"Iteracao": iteracoes, "Cenarios": cenarios, "Estagio": estagios, "Codigo": codigos, "Violacao": violacoes, "Unidade": unidades, "Submercado": submercados, "Patamar": patamares})
